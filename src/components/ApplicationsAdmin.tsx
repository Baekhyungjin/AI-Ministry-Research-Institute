...
import { ApplicationItem, ApplicationStatus, SchedulePaymentStatus } from '@/lib/types';
import { useRecords } from '@/lib/use-records';
import { downloadCsv } from '@/lib/csv';

const statusLabels: Record<ApplicationStatus, string> = { new: '새 접수', contacted: '연락 완료', confirmed: '확정', closed: '종료' };
const paymentLabels: Record<SchedulePaymentStatus, string> = { not_required: '결제 없음', pending: '입금 대기', confirmed: '입금 확인', cancelled: '입금 취소' };
...
export default function ApplicationsAdmin() {
  ...
  const items = [...records].filter((item) => (filter === 'all' || item.status === filter) && (!normalizedQuery || [item.name, item.church, item.email, item.phone, item.message, item.scheduleTitle, item.depositorName].some((value) => value?.toLowerCase().includes(normalizedQuery)))).sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  ...
  async function changePaymentStatus(item: ApplicationItem, paymentStatus: SchedulePaymentStatus) {
    setMessage('');
    try {
      await updateRecord<ApplicationItem>('applications', item.id, { paymentStatus });
      setMessage('입금 상태를 저장했습니다.');
    } catch {
      setMessage('입금 상태를 저장하지 못했습니다. 다시 시도해 주세요.');
    }
  }

  function exportItems() {
    downloadCsv(`문의-신청-${new Date().toISOString().slice(0, 10)}.csv`, ['구분', '상태', '성함', '교회·기관', '직분', '연락처', '이메일', '희망 날짜', '신청 일정', '입금자명', '신청금액', '입금상태', '채팅방 확인', '내용', '접수일'], items.map((item) => [item.kind, statusLabels[item.status], item.name, item.church, item.role, item.phone, item.email, item.requestedDate, item.scheduleTitle, item.depositorName, item.paymentAmount, paymentLabels[item.paymentStatus ?? 'not_required'], item.chatJoined ? '확인' : '', item.message, item.createdAt]));
  }

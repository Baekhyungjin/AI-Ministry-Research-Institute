...
export default function ApplicationForm() {
  ...
  return (
    <form className="application-form" onSubmit={handleSubmit}>
      ...
      <div className="form-section"><span className="form-step">03</span><div><h2>필요한 내용을 들려주세요.</h2><label className="textarea-label">문의 내용<textarea name="message" rows={7} required placeholder="현재 상황, 예상 인원, 희망 일정, 필요한 교육 내용을 적어 주세요." /></label><input className="form-trap" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" /><label className="consent-row"><input type="checkbox" name="consent" required /><span>문의 처리 및 연락을 위한 개인정보 수집·이용에 동의합니다.</span></label>{error && <div className="form-error" role="alert">{error}</div>}<button className="btn btn-primary submit-button" disabled={saving || Boolean(kind === 'lecture' && requestedDate && schedules.some((item) => item.date === requestedDate))}>{saving ? '접수 중…' : '문의 및 신청 접수하기'}</button></div></div>
    </form>
  );
}

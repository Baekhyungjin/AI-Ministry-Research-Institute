...
function ResourceImageField({ currentUrl, title, helperText = 'JPG, PNG, WEBP, GIF · 최대 5MB' }: { currentUrl?: string | null; title: string; helperText?: string }) {
  return <>
    <label className="content-image-field">대표 이미지<input type="file" name="image" accept="image/jpeg,image/png,image/webp,image/gif" /><small>{helperText}</small></label>
...
function GptAdmin() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    } catch { if (imageUrl && imageUrl !== editing?.imageUrl) await deleteManagedImage(imageUrl).catch(() => undefined); setMessage('GPT를 저장하지 못했습니다.'); } finally { setSaving(false); }
  }
...
function AppAdmin() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try { if (editing) { await updateRecord<AppItem>('apps', editing.id, values); if (editing.imageUrl && editing.imageUrl !== imageUrl) void deleteManagedImage(editing.imageUrl); setEditing(null); setMessage('앱 변경 내용을 저장했습니다.'); } else { await createRecord<AppItem>('apps', { id: createId('app'), ...values, createdAt: new Date().toISOString() }); e.currentTarget.reset(); setMessage('새 앱을 저장했습니다.'); } } catch { if (imageUrl && imageUrl !== editing?.imageUrl) await deleteManagedImage(imageUrl).catch(() => undefined); setMessage('앱을 저장하지 못했습니다.'); } finally { setSaving(false); }
  }
